...
def get_final_criterion_status(dprime_values):
    """
    Determine final learned-criterion status using only the existing d-prime rule.

    Inputs:
    - dprime_values: sequence of floats, one value per trial

    Returns:
    - str: "criterion met" or "criterion not met"
    """
    criterion_met, _ = check_consecutive_dprime(dprime_values)
    if criterion_met:
        return "criterion met"
    return "criterion not met"


def safe_stop_plot_process(plot_process):
    """
    Stop the active plotting process, if one exists.

    Parameters:
    - plot_process: multiprocessing.Process or None. The process used for the
      most recent real-time progress plot.

    Returns:
    - None. The process is terminated in place if it is still alive.
    """
...
def safe_end_task_session(task):
    ...
    Parameters:
    - task: go_nogo_firstrule instance or None. The task object owns the cues,
...
def safe_save_session_info(session_info):
    """
    Save session metadata if the output basename has already been created.

    Parameters:
    - session_info: dict or None. Session metadata containing file_basename.

    Returns:
    - None. Writes .mat and .pkl files when the required metadata exists.
    """
    if not session_info or 'file_basename' not in session_info:
        return
    ...
    try:
        ...
        print("Session info save warning: " + str(save_error))
...
if __name__ == "__main__":
    ...
    # graceful exit
        ...
        if dprimebinp:
            final_criterion_status = get_final_criterion_status(dprimebinp)
            print(final_criterion_status)
            logging.info(final_criterion_status)

        # save dicts to disk
        safe_save_session_info(session_info)
        pygame.quit()
