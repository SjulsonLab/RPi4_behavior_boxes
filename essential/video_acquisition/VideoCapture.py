...
class VideoCapture():
    ...
    """
    def __init__(self, IP_address_video, video_name, base_pi_dir,
                 local_storage_dir, frame_rate=30):
    ...
    def video_preview_only(self):
        ...
        try:
            ...
            print(Fore.RED + "\n CTRL + C to quit previewing" + Style.RESET_ALL)

            try:
                subprocess.run([
                    "ssh",
                    "pi@" + IP_address_video,
                    "/home/pi/RPi4_behavior_boxes/start_preview.py"
                ])
            ...
            print(
                Fore.GREEN
                + "\nKilling the preview process."
                + Style.RESET_ALL
            )
            os.system(
                "ssh pi@" + IP_address_video
                + " '/home/pi/RPi4_behavior_boxes/stop_preview.sh'"
            )
            time.sleep(2)
    ...
    def video_start(self):
        ...
        print(
            Fore.YELLOW
            + "Killing any python process prior to this session!\n"
            + Style.RESET_ALL
        )
        ...
        try:
            ...
            print(
                Fore.RED
                + "\n CTRL + C to quit previewing and start recording"
                + Style.RESET_ALL
            )
            ...
            try:
                ...
                print(
                    Fore.GREEN
                    + "\nPreview interrupted. Continuing to recording..."
                    + Style.RESET_ALL
                )
            ...
            print(
                Fore.GREEN
                + "\nKilling any python process before start recording!"
                + Style.RESET_ALL
            )
            ...
            print(
                Fore.RED
                + Style.BRIGHT
                + "Please check if the preview screen is on! "
                  "Cancel the session if it's not!"
                + Style.RESET_ALL
            )
    ...
    def video_stop(self):
        ...
        try:
            ...
            # Create a directory for storage on the hard drive mounted
            # on the box behavior
            hd_dir = self.local_storage_dir

            # This is to avoid copying the data into a HDD while ephys
            # or other process might still going on, thus corrupting
            # ongoing data writing
            input(
                "Press any key to continue with copying the videos "
                "to the local storage..."
            )

            # Move the video + log from the box_video SD card to the
            # box_behavior external hard drive
            os.system(
                "rsync -av --progress --remove-source-files pi@"
                + IP_address_video
                + ":"
                + dir_name
                + "/ "
