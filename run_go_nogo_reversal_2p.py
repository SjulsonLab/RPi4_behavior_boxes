...
def get_final_criterion_message(dprime_values):
    """Return the final d-prime learning status string for logging and console output.

    Args:
        dprime_values (Sequence[float]): Per-trial d-prime values.

    Returns:
        str: "criterion met" if the existing d-prime rule is satisfied, else
            "criterion not met".
    """
...
if __name__ == "__main__":
    ...
    # graceful exit
        ...
        task.end_session()
        criterion_message = get_final_criterion_message(dprimebinp)
