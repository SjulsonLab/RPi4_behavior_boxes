...
BASELINE_DURATION_SECONDS = 15 * 60
...
def run_timed_baseline(
    task,
    duration_seconds=BASELINE_DURATION_SECONDS,
    time_fn=time.time,
    sleep_fn=time.sleep,
):
    """Run a no-cue baseline period inside the existing open session.

    Inputs
    ------
    task : go_nogo_firstrule-like object
        Active task object whose session/video/treadmill acquisition should
        remain open during baseline.
    duration_seconds : float, optional
        Requested baseline duration in seconds. Default is 900 seconds.
    time_fn : callable, optional
        Function returning current absolute UNIX time in seconds. Defaults to
        ``time.time``; injectable for tests.
    ...
    -------
    dict
        Baseline metadata with UNIX start/end times, requested and actual
        durations in seconds, completion/interruption booleans, and lick times
        in seconds relative to baseline start.
    """
    clear_behavior_cues_for_baseline(task)

    baseline_start_time = time_fn()
    baseline_end_target = baseline_start_time + duration_seconds
    baseline_lick_times = []
    baseline_interrupted = False

    logging.info(str(baseline_start_time) + ", baseline_start")
    print(Fore.RED + Style.BRIGHT + "Starting 15 minute baseline. Press Ctrl+C again to stop baseline early." + Style.RESET_ALL)

    try:
        while time_fn() < baseline_end_target:
        ...
        baseline_interrupted = True
        logging.info(str(time_fn()) + ", baseline_interrupted")
        print(Fore.RED + Style.BRIGHT + "Baseline interrupted. Exiting cleanly." + Style.RESET_ALL)
    ...
    baseline_completed = (not baseline_interrupted) and (baseline_end_time >= baseline_end_target)
    if baseline_completed:
        logging.info(str(baseline_end_time) + ", baseline_completed")
    logging.info(str(baseline_end_time) + ", baseline_end")

    return {
        "baseline_requested": True,
        "baseline_req_dur_s": float(duration_seconds),
        "baseline_start_unix": float(baseline_start_time),
        "baseline_end_unix": float(baseline_end_time),
        "baseline_actual_dur_s": float(baseline_end_time - baseline_start_time),
        "baseline_completed": bool(baseline_completed),
        "baseline_interrupted": bool(baseline_interrupted),
...
if __name__ == "__main__":
    ...
    try:
        ...
        session_info['behavior_phase'] = training_phase
        if training_phase in {'allgo', 'firstrule'}:
            session_info['session_label'] = training_phase + '_with_baseline'
        else:
            session_info['session_label'] = training_phase
        session_info['file_basename'] = session_info['mouse_name'] + "_" + session_info['session_label'] + "_" + session_info['datetime']
        logging.basicConfig(
            level=logging.INFO,
    ...
    # graceful exit
        ...
        if should_run_baseline:
            logging.info(str(time.time()) + ', behavior_phase_exit')
            baseline_metadata = run_timed_baseline(
                task,
                duration_seconds=BASELINE_DURATION_SECONDS,
            )
        ...
        # save dicts to disk
        if session_info.get('file_basename', None) is not None:
            scipy.io.savemat(session_info['file_basename'] + '_session_info.mat', {'session_info' : session_info})
            pickle.dump( session_info, open( session_info['file_basename'] + '_session_info.pkl', "wb" ) )
        pygame.quit()
