...
# This version has multiprocessing for plotting, same as v2
# The difference compared to v2: REWARD ALWAYS COMES OUT AT THE END OF THE FIRST SECOND!!!!!
...
class go_nogo_task(object):
    ...
    def run_go(self):
        ...
        if self.state == "standby":
            ...
            # if lick is detected, transition to temp1 state
            if event_name =="left_IR_entry":
                logging.info(str(time.time()) + ", reward initiated!")
                self.start_temp1()  # trigger transition to temp1
