...
class VisualStim(object):
    ...
    def get_screen_info(self):
        """Return RPG Screen setup matching the grating file.

        Inputs
        ------
        None. Uses ``self.session_info`` with ``vis_gratings`` paths and
        ...
        """
        if self.session_info["vis_gratings"]:
            try:
                return screen_args_for_grating(
                    self.session_info["vis_gratings"][0],
                    self.session_info["gray_level"],
                )
