...
class FlipperOutput(DigitalOutputDevice):
    ...
    def __init__(self, flipper_filename, pin=None):
        if pin is None:
            # raise ValueError("Pin must be specified for FlipperOutput")
            pin = 4  # Default pin, can be changed as needed

        super(FlipperOutput, self).__init__(pin=pin)
        self._flip_thread = None
        self._running = False
        self._stop_flag = Event()
        self._flipper_filename = flipper_filename
