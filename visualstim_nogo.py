...
class VisualStim_nogo(object):
    ...
    def get_screen_info(self):
        """Return RPG Screen setup matching the no-go grating file.

        Inputs
        ------
        None. Uses ``self.session_info`` with ``vis_gratings_nogo`` paths and
        ...
        """
        if self.session_info["vis_gratings_nogo"]:
            try:
                return screen_args_for_grating(
                    self.session_info["vis_gratings_nogo"][0],
                    self.session_info["gray_level"],
                )
    ...
    def load_session_gratings(self):
        for filepath in self.session_info["vis_gratings_nogo"]:
