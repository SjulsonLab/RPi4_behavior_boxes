...
class BehavBox(object):
    ...
    def video_start(self):
        ...
        try:
            ...
            # keep the working preview flow from DT_test; DT removed it and replaced it with pkill
            if self.run_preview:
                self._run_remote(
                ...
                print(Fore.YELLOW + "Preview skipped because session_info['run_preview'] is False." + Style.RESET_ALL)
            ...
            print(
                Fore.RED
                + Style.BRIGHT
                + "Please check if the preview screen is on! Cancel the session if it's not!"
                + Style.RESET_ALL
            )
